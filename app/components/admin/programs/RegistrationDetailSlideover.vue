...
const midtransOrderId = computed(() => {
  const payment = props.registration?.payments?.[0]
  return payment?.transaction?.midtrans_data?.midtrans_order_id ?? payment?.transaction?.transaction_code ?? null
})

const midtransOrderUrl = computed(() => {
  const orderId = midtransOrderId.value
  return orderId
    ? `https://dashboard.midtrans.com/beta/transactions?type=order_id&query=${orderId}`
    : null
})
</script>
  ...
  >
    <template #body>
      <div v-if="registration">
        ...
        <div class="p-6">
          <p class="text-xs font-bold uppercase tracking-wide mb-5">Personal Information</p>
          <div class="space-y-5">
        ...
        <div v-if="registration.payments?.[0]" class="p-6">
          <p class="text-xs font-bold uppercase tracking-wide mb-5">Payment Information</p>
          <div class="space-y-5">
            <div class="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
            ...
            >
              ...
              <a
                v-if="midtransOrderUrl"
                :href="midtransOrderUrl"
                target="_blank"
                rel="noopener"
                class="inline-flex items-center gap-1 text-primary hover:underline"
              >
                <UIcon name="i-ph-arrow-square-out-bold" class="size-4 shrink-0" />
                <span class="truncate" :title="midtransOrderId ?? undefined">{{ midtransOrderId }}</span>
              </a>
